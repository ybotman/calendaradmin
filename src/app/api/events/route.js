...
/**
 * GET handler for events API endpoint - proxies to backend
 * 
 * @param {Request} request - The incoming request
 * @returns {Promise<Response>} JSON response with events data
 */
export async function GET(request) {
  console.log('Events API endpoint called - Proxying to backend');
  ...
  try {
    ...
    const apiUrl = `${backendUrl}/api/events?${searchParams.toString()}`;
    ...
    try {
      ...
      console.log(`Successfully received data from backend with ${data.events?.length || 0} events`);
...
/**
 * POST handler for creating events - proxies to backend
 */
export async function POST(request) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/events/post`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
